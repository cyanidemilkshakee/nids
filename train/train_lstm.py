"""LSTM (Long Short-Term Memory) training script for AI NIDS.
Designed for ~1 hour training with real-time progress monitoring.
Uses LSTM architecture for sequential network traffic pattern analysis.

Environment Variables:
  PRIMARY_DATA=unsw_selected_features.csv
  OUTPUT_MODEL=saved_model_lstm.h5
  MAX_ROWS=180000
  TEST_SIZE=0.2
  EPOCHS=40
  BATCH_SIZE=256
...
if gpus:
    try:
        ...
        print(f"[LSTM] GPU available: {len(gpus)} device(s)")
    except RuntimeError as e:
        print(f"[LSTM] GPU setup error: {e}")
else:
    print("[LSTM] No GPU found, using CPU")
...
OUTPUT_MODEL = os.environ.get('OUTPUT_MODEL', 'saved_model_lstm.h5')
...
log = setup_logging("train.lstm")
log.info("=" * 80)
log.info("Starting LSTM Training for AI-NIDS")
log.info("=" * 80)
log_kv(log, PRIMARY_DATA=PRIMARY_FILE, OUTPUT_MODEL=OUTPUT_MODEL)
log_kv(log, MAX_ROWS=MAX_ROWS, TEST_SIZE=TEST_SIZE, EPOCHS=EPOCHS, BATCH_SIZE=BATCH_SIZE, LSTM_UNITS=LSTM_UNITS)
...
# For network traffic, we treat each feature as a timestep
X_train_reshaped = X_train.reshape(X_train.shape[0], X_train.shape[1], 1)
X_test_reshaped = X_test.reshape(X_test.shape[0], X_test.shape[1], 1)
log.info(f"Reshaped for LSTM: train={X_train_reshaped.shape}, test={X_test_reshaped.shape}")
...
def build_lstm_model(input_shape, num_classes, lstm_units=128):
...
checkpoint_path = os.path.join(MODEL_DIR, 'lstm_checkpoint.h5')
...
# Train model
log.info(f"Starting LSTM training for {EPOCHS} epochs...")
...
# Save model
log.info(f"Saving LSTM model to {os.path.join(MODEL_DIR, OUTPUT_MODEL)}")
...
json_path = os.path.join(MODEL_DIR, 'lstm_architecture.json')
with open(json_path, 'w') as f:
    f.write(model_json)
log.info(f"Model architecture saved -> {json_path}")

# Save training history
history_path = os.path.join(MODEL_DIR, 'lstm_training_history.json')
history_dict = {
...
log.info("="*80)
log.info("LSTM training pipeline completed successfully!")
log.info("="*80)
