...
if os.path.exists(synth_path):
	try:
		...
		print(f"[INFO] Augmented with synthetic samples: {len(synth_df)} rows -> total {len(df)}")
	except Exception as e:
		print(f"[WARN] Failed to use synthetic data: {e}")
else:
	print("[INFO] No synthetic augmentation file found; proceeding with primary dataset only.")
...
if os.environ.get('TRAIN_GAN', '0') == '1':
	try:
		...
		if minority_classes:
			print(f"[GAN] Minority classes targeted: {minority_classes}")
			noise_dim = 32
			def build_generator():
			...
			for cls in minority_classes:
				...
				if target <= 0:
					continue
				print(f"[GAN] Training class {cls} to generate ~{target} samples")
				...
				for epoch in range(EPOCHS):
					...
					if (epoch+1) % 5 == 0:
						print(f"[GAN][Class {cls}] Epoch {epoch+1}/{EPOCHS} d_loss_real={d_loss_real:.3f} d_loss_fake={d_loss_fake:.3f} g_loss={g_loss:.3f}")
			...
			print(f"[GAN] Augmented dataset size after GAN: {len(df)}")
		else:
...
if len(df) > MAX_ROWS:
    ...
    print(f"[INFO] Final dataset capped {before} -> {len(df)} rows (MAX_ROWS={MAX_ROWS})")
...
print(f"[INFO] Class weights: {cw_dict}")
...
print(f"[INFO] Metrics updated -> {metrics_path}")
