"""CNN (Convolutional Neural Network) training script for AI NIDS.
Designed for ~1 hour training with real-time progress monitoring.
Uses 1D CNN architecture for network traffic classification.

Environment Variables:
  PRIMARY_DATA=unsw_selected_features.csv
  OUTPUT_MODEL=saved_model_cnn.h5
  MAX_ROWS=200000
  TEST_SIZE=0.2
  EPOCHS=50
  BATCH_SIZE=256
...
if gpus:
    try:
        ...
        print(f"[CNN] GPU available: {len(gpus)} device(s)")
    except RuntimeError as e:
        print(f"[CNN] GPU setup error: {e}")
else:
    print("[CNN] No GPU found, using CPU")
...
OUTPUT_MODEL = os.environ.get('OUTPUT_MODEL', 'saved_model_cnn.h5')
...
log = setup_logging("train.cnn")
log.info("=" * 80)
log.info("Starting CNN Training for AI-NIDS")
log.info("=" * 80)
log_kv(log, PRIMARY_DATA=PRIMARY_FILE, OUTPUT_MODEL=OUTPUT_MODEL)
log_kv(log, MAX_ROWS=MAX_ROWS, TEST_SIZE=TEST_SIZE, EPOCHS=EPOCHS, BATCH_SIZE=BATCH_SIZE)
...
y_train_labels = y[:len(X_train)]  # For class weights
y_test_labels = y[len(X_train):]

log_kv(log, train_shape=X_train.shape, test_shape=X_test.shape)

# Reshape for 1D CNN (samples, timesteps, features)
# We'll treat each sample as a sequence with 1 timestep per feature
X_train_reshaped = X_train.reshape(X_train.shape[0], X_train.shape[1], 1)
X_test_reshaped = X_test.reshape(X_test.shape[0], X_test.shape[1], 1)
log.info(f"Reshaped for CNN: train={X_train_reshaped.shape}, test={X_test_reshaped.shape}")
...
# Build CNN model
log.info("Building 1D CNN architecture...")
...
model = build_cnn_model(input_shape=(X_train_reshaped.shape[1], 1), num_classes=num_classes)
...
checkpoint_path = os.path.join(MODEL_DIR, 'cnn_checkpoint.h5')
...
# Train model
log.info(f"Starting CNN training for {EPOCHS} epochs...")
...
# Save model
log.info(f"Saving CNN model to {os.path.join(MODEL_DIR, OUTPUT_MODEL)}")
...
json_path = os.path.join(MODEL_DIR, 'cnn_architecture.json')
...
log.info("="*80)
log.info("CNN training pipeline completed successfully!")
log.info("="*80)
